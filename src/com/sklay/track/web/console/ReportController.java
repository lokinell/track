...
public class ReportController extends BaseController
{
    ...
    public String event(Model model, @RequestParam(value = "appId", required = false)
    String appId, @RequestParam(value = "index", defaultValue = "1")
    int index, @RequestParam(value = "size", defaultValue = "20")
    int size, String path, Long userId, String jsonQuery, boolean unwind, Date startDate, Date endDate)
        throws Exception
    {
        ...
        if (StringUtils.isEmpty(jsonQuery))
        {
            ...
            events =
                mongo.find(query.with(new Sort(new Sort.Order(Sort.Direction.DESC, "beginAt")))
                    .skip(offset)
                    .limit(size), Event.class);
        }
        else
        {
            try
        ...
        if (CollectionUtils.isNotEmpty(events))
        {
            ...
            {
                errors.put(event.getId(),
                    mongo.count(Query.query(Criteria.where("eventId").is(event.getId())), ErrorLog.class));
            }
        }
    ...
    public String action(@PathVariable("id")
    ...
    public String error(@PathVariable("id")
    String id, Model model)
        throws Exception
    {
