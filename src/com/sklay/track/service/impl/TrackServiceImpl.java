...
public class TrackServiceImpl implements ServletContextAware, InitializingBean, TrackService
{
    ...
    @Override
    public void boot(String appId, String version, HttpServletRequest request, HttpServletResponse response)
        throws IOException
    {
        App app;
        try
        ...
        if (debug)
        {
            writer.write(IOUtils.toString(servletContext.getResourceAsStream("/WEB-INF/js/boot-" + version + ".js"),
                Constants.CHARSET));
        }
        else
        {
            ...
            if (body == null)
            {
                ...
                {
                    body =
                        IOUtils.toString(servletContext.getResourceAsStream("/static/js/boot-" + version + ".js"),
                            Constants.CHARSET);
                }
                catch (IOException e)
                {
    ...
    @Override
    public void collect(String appId, HttpServletRequest request, HttpServletResponse response)
        throws IOException
    {
    ...
    private String assertNotEmpty(String key, String value)
    {
        ...
        {
            throw new IllegalArgumentException("this String argument [" + key
    ...
    public String trackEvent(String eventId, String sessionId, String clientId, Integer type, JSONObject data, App app,
        HttpServletRequest request)
    {
        ...
        {
            trackDataService.saveEvent(trackDataExtractor.extractEvent(trackDataService.getEvent(eventId),
                data,
                app,
                request));
        }
    ...
    public List<Action> trackActions(String eventId, JSONArray arr, App app, HttpServletRequest request)
    {
        if (arr != null)
        {
            ...
            for (int i = 0, len = arr.size(); i < len; i++)
            {
                ...
                actions.add(trackDataService.saveAction(trackDataExtractor.extractAction(action,
                    actionJSON,
                    app,
                    request)));
            }
    ...
    public List<ErrorLog> trackErrorLogs(String eventId, JSONArray arr, App app, HttpServletRequest request)
    {
        if (arr != null)
        {
            ...
            for (int i = 0, len = arr.size(); i < len; i++)
            {
                ...
                errorLogs.add(trackDataService.saveErrorLog(trackDataExtractor.extractErrorLog(errorLog,
                    errorLogJSON,
                    app,
                    request)));
            }
    ...
    public void outputEmptyGif(HttpServletResponse response)
        throws IOException
    {
